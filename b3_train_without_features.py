...
default_script=os.path.basename(__file__)
default_filename=f"{datetime.now().strftime('%Y-%m%d-%H%M%S')}_{default_script}"



save_folder=f'data/sample/{default_filename}'
...
# %%
torch.save(model,f'data/model_{datetime.now().timestamp()}_{round(val_loss.avg,4)}.pt')
# %%
